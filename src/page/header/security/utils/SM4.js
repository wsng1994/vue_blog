 ...
 * @return
 */
export function decrypt(plaintext, key, flag) {
  let ciphertext = []
  let k = 0
  let plainLen = plaintext.length
  while (k + 16 <= plainLen) {
    let cellPlain = [] // 16-byte
    for (let i = 0; i < 16; i++) {
      cellPlain[i] = plaintext[k + i]
    }
    let cellCipher = sms4(cellPlain, 16, key, [], flag)
    for (let i = 0; i < cellCipher.length; i++) {
...
export function encrypt(plaintext, key) {
  ...
  // 16位整数倍
  //		let byteLength = Code.str2bytes(plaintext).length;
  let byteLength = Code.hexStr2bytes(plaintext).length
  for (let i = byteLength % 16; i < 16; i++) {
    plaintext += " "
  }
  // console.log("sm4 src:[" + plaintext + "]");
  //		return crypt(Code.str2bytes(plaintext), key, ENCRYPT);
  return decrypt(Code.hexStr2bytes(plaintext), key, ENCRYPT)
}
